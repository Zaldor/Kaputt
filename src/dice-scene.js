...
export class DiceScene {
  constructor(container) {
    ...
    const light = new THREE.DirectionalLight(0xfff0d0, 5.5);
    light.position.set(-1.5, 5, 10);
    light.castShadow = true;
    light.shadow.mapSize.set(1024, 1024);
    Object.assign(light.shadow.camera, { left: -4, right: 4, top: 3, bottom: -3 });
    light.shadow.normalBias = .02; light.shadow.radius = 3;
    this.scene.add(light);
    const fill = new THREE.DirectionalLight(0xd0e0ff, .6);
    fill.position.set(5, 1, 4); this.scene.add(fill);
    const rim = new THREE.DirectionalLight(0xffeedd, 1.8);
    rim.position.set(-3, 3, -4); this.scene.add(rim);
    const floor = new THREE.Mesh(new THREE.PlaneGeometry(30, 30), new THREE.ShadowMaterial({ opacity: .45 }));
    ...
    this.ivory = new THREE.MeshPhysicalMaterial({ color: 0xfff0c8, roughness: .28, clearcoat: .9, clearcoatRoughness: .08, reflectivity: .6 });
    this.orange = new THREE.MeshPhysicalMaterial({ color: 0xe86800, roughness: .22, clearcoat: .95, clearcoatRoughness: .05, reflectivity: .7 });
    this.ink = new THREE.MeshPhysicalMaterial({ color: 0x081028, roughness: .15, clearcoat: .8 });
    this.rim = new THREE.MeshStandardMaterial({ color: 0xd0c8a0, roughness: .25, metalness: .1 });
